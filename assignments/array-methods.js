...
//behalf for some much needed updates to their facilities.  Each business has assigned a representative to attend the event along with a 
//small donation.
...
// Problem 1
//The race director needs the emails of all the runners.
let runnerEmails=[];

runners.forEach(function (runner) {
    runnerEmails.push(runner.email);
  });

  console.log(runnerEmails);

// Problem 2
//Change all last names to lowercase.
let noCaps= [];
...
// Problem 3
//Find all the runners that work for Podcat
let podcatEmployee = [];

podcatEmployee = runners.filter(function (runner) {

    if (runner.company_name === 'Podcat') {
  
      runners.filter(function (runner) {
        return runner.company_name === 'Podcat';
      });
      return runner;
    }
